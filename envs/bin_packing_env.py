...
class BinPacking3DEnv(gym.Env):
    ...
    def reset(self, seed=None, options=None):
        ...
        save_bin_state(
            ...
            new_box=self.box_instruction,
            bin_dims=[self.bin_width, self.bin_height, self.bin_depth],
    ...
    def step(self, action):
        ...
        if not done:
            ...
            self.placed_boxes.append({
                ...
                "color": self.box_instruction["color"]
            })
    ...
    def render(self):
        ...
        for box in self.placed_boxes:
            ...
            sx, sy, sz = box["size"]
            self.ax.bar3d(px, py, pz, sx, sy, sz, color=box["color"], alpha=0.6)

        # Draw current moving box
        x, y, z = self.box_position
        dx, dy, dz = self.box_instruction["size"]
        self.ax.bar3d(x, y, z, dx, dy, dz, color=self.box_instruction["color"], alpha=0.8)
